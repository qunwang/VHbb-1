...
ClassImp(RooSpinZero_3D_WH_pp) 

RooSpinZero_3D_WH_pp::RooSpinZero_3D_WH_pp(const char *name, const char *title, 
                                           RooAbsReal& _h1,
                                           RooAbsReal& _h2,
                                           RooAbsReal& _Phi,
                                           RooAbsReal& _m,
...
RooSpinZero_3D_WH_pp::RooSpinZero_3D_WH_pp(const RooSpinZero_3D_WH_pp& other, const char* name) :  
...
Double_t RooSpinZero_3D_WH_pp::evaluate() const 
{ 
...
//    fm0 = fm0 / ftotal;
    ...
    double g1_SM = 1.;
    double a1_SM = g1_SM*mZ*mZ/(mX*mX);
    double a1Im_SM = 0.;
    double a2_SM = 0.;
    double a2Im_SM = 0.;
    double a3_SM = 0.;
    ...
    angularPartIntegral += (32*f00_SM*Power(Pi(),2))/9.;
    angularPartIntegral += (32*fp0_SM*Power(Pi(),2))/9.;
    angularPartIntegral += (32*fm0_SM*Power(Pi(),2))/9.;
    double phaseSpaceFactor = sigma_SM_psf/angularPartIntegral;
    //double phaseSpaceFactor = sigma_SM_psf;    
...
Int_t RooSpinZero_3D_WH_pp::getAnalyticalIntegral(RooArgSet& allVars, RooArgSet& analVars, const char* /*rangeName*/) const
{
...
Double_t RooSpinZero_3D_WH_pp::analyticalIntegral(Int_t code, const char* rangeName) const
{
...
vector<TLorentzVector> RooSpinZero_3D_WH_pp::Calculate4Momentum(float Mx,float M1,float M2,float theta,float theta1,float theta2,float _Phi1_,float _Phi_) const 
{
...
float RooSpinZero_3D_WH_pp::partonicLuminosity(float mVal, float YVal, float sqrtsVal) const
{
	...
	Double_t FuncABu = FuncAu1/xa*FuncBu1/xb+FuncAu2/xa*FuncBu2/xb; // uubar + sym
	...
	Double_t FuncABd = FuncAd1/xa*FuncBd1/xb+FuncAd2/xa*FuncBd2/xb;  // ddbar + sym
	...
	Double_t FuncABc = Funcsa*Funcsb/xa/xb; // ccbar + sym
	Double_t FuncABs = Funcca*Funccb/xa/xb; // ssbar + sym
	Double_t FuncABb = Funcba*Funcbb/xa/xb; // bbbar + sym
...
//
//	    FuncABu = 2*(FuncAu1/xa0*FuncBd1/xa0);	    
//        FuncABd = 2*(FuncAd1/xa0*FuncBu1/xa0);
//        
//	    //sea
...
//	    FuncABc = Funcsa*Funcca/xa0/xa0;
//	    FuncABs = Funcca*Funcsa/xa0/xa0;
